...
const wastewaterOptions: Record<WasteMode, {title:string; status:string; body:string; test:string}> = {
  carriage: {
    ...
    body: 'Local pressure sewers feed ring-side holding tanks. At several fixed stations, a carriage matches the ring’s 0.5 m/s speed, makes a verified dry-break connection, pumps during a limited travel stroke, isolates, disconnects and returns. Separate equipment and galleries serve potable water.',
...
const operatingStates = [
  ...
  ['Seismic / wind hold', 'Ring stops at any angle; lateral guides and hold-downs engaged', 'Fixed emergency routes and local essential services remain available']
];
...
export default function InfrastructurePage() {
  ...
  return <main id="main-content" className="editorial">
    <header className="page-header systems-header">
      <p className="eyebrow">ROTUNDA / HOW THE CITY WORKS · REVISION 0.2</p>
      <h1>The rings move.<br/><em>The ground does not.</em></h1>
      ...
      <nav className="page-subnav" aria-label="Infrastructure sections"><a href="#motion">Motion</a><a href="#water">Freshwater</a><a href="#sewage">Wastewater</a><a href="#power">Power</a><a href="#interfaces">Handoff</a></nav>
    </header>

    <section id="motion" className="section technical-section">
    ...
    <section id="water" className="section technical-section">
      ...
      <div className="table-scroll"><table className="technical-table option-table"><thead><tr><th>Route</th><th>Assessment</th><th>Decision</th></tr></thead><tbody>
        <tr><td>Top-fed / “pour from above”</td><td>Requires lifting the entire flow above 60 m, exposes water to contamination and weather, and still needs storage and pressure control.</td><td><span className="status no">Not selected</span></td></tr>
    ...
    <section id="sewage" className="section technical-section">
      ...
      <div className="utility-tabs" aria-label="Compare wastewater transfer options">{Object.entries(wastewaterOptions).map(([key, option])=><button key={key} aria-pressed={wasteMode===key} onClick={()=>setWasteMode(key as WasteMode)}>{option.title}</button>)}</div>
      ...
      <UtilitySection mode={wasteMode} playing={wastePlaying}/>
    ...
    <section id="power" className="section technical-section">
